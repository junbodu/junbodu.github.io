...
## This checkout is a personalized fork

The remote is `junbodu/dujunbo.github.io` — a **deployed personal site**, not the upstream starter. Two consequences that override the imported `AGENTS.md` / `.github/copilot-instructions.md`:

- **`baseurl` is blank** (`_config.yml`: `url: https://junbodu.github.io`). Those files' validated command set says `--baseurl /al-folio` and `curl .../al-folio/` — that is upstream's GitHub-Pages-project-path setup and does **not** apply here. Build with a bare `bundle exec jekyll build`, browse at `http://localhost:4000/`. Everything else in their command set is accurate.
- **Content is still largely upstream demo content** (`_pages/about_einstein.md`, `_projects/1_project.md`, `_books/the_godfather.md`, `_news/announcement_1.md`), while `_config.yml` identity is already personalized. Editing/removing sample content is expected work here and is not a boundary violation — unlike editing runtime, which still is.
...
The `al-*` / `al_folio_*` gems are developed as **sibling repos on disk** at `~/Documents/dev/al-org/<repo>` — repo dir uses hyphens (`al-folio-core`), gem/plugin id uses underscores (`al_folio_core`). That path is **not checked out on this machine**, so gems here resolve from RubyGems and gem source is only readable via `bundle show <gem>`. To test a gem fix against this site, point the `Gemfile` at a local clone: `gem "al_folio_core", path: "../al-folio-core"` (or `git:`/`branch:`), then `bundle install`.
...
- **Two parallel lists must stay in sync:** `Gemfile` (pinned versions, e.g. `al_folio_core '= 1.0.11'`) and `_config.yml`'s `plugins:` list. Adding/removing a plugin means editing both.
...
bundle exec jekyll serve                      # dev server → http://localhost:4000/   (baseurl is blank here — see below)
bundle exec jekyll build                      # production build to _site/ (what deploy.yml and bin/cibuild run)
bash test/integration_distill.sh             # run ONE integration test (any of the six)
...
`docker compose up -d` bind-mounts the repo to `/srv/jekyll` and runs `bin/entry_point.sh`, which serves with `--force_polling --destination /tmp/_site`. The build output deliberately goes to **container-local `/tmp/_site`, not the bind-mounted `_site`** — writing `_site` back across the host bind mount caused write deadlocks. The container also `inotifywait`s `_config.yml` and restarts Jekyll on change (config edits aren't hot-reloaded by `--watch`). Verify at the site root: `curl -fsS http://127.0.0.1:8080/` (upstream docs say `/al-folio/`; this fork's baseurl is blank). `docker-compose-slim.yml` pulls a prebuilt `:slim` image instead of building locally.

## CI gates and the style contract

`npm run lint:style-contract` (`test/style_contract.js`) is the automated enforcement of the thin-starter boundary, and it will fail CI if you cross it: the starter must **not** define `build:css`/`build:tailwind` npm scripts, must **not** own `_includes`/`_layouts`/`_sass`/`_scripts`/`assets/tailwind`/`tailwind.config.js`/icon-font artifacts, must keep `theme: al_folio_core` and the required plugins in `_config.yml`, and must keep the `third_party_libraries` SRI pins (`fontawesome`/`academicons`/`scholar-icons` `integrity.css`, plus `tikzjax`/`tocbot` keys) and the **exact** `al_math '= 1.0.1'` Gemfile pin (a `:git =>` pin fails too); `assets/webfonts` is forbidden alongside the other component paths. Other gates: `unit-tests.yml` (style contract + the six integration scripts), `visual-regression.yml` (Playwright chromium+webkit, diffs candidate against a v0.16.3 baseline served on :4100 via `BASELINE_URL`), `upgrade-check.yml` (`al-folio upgrade audit`), `prettier.yml`. Prettier uses `@shopify/prettier-plugin-liquid` with `printWidth: 150`; run `npm run lint:prettier` before pushing.
